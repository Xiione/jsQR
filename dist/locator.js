...
function scorePattern(point, ratios, matrix) {
    try {
        ...
        const avgSize = (horzError.averageSize +
...
function locate(matrix) {
    ...
    for (let y = 0; y <= matrix.height; y++) {
        ...
        for (let x = -1; x <= matrix.width; x++) {
            ...
            if (v === lastBit) {
                ...
                const validFinderPattern = Math.abs(scans[0] - averageFinderPatternBlocksize) <
                ...
                const validAlignmentPattern = Math.abs(scans[2] - averageAlignmentPatternBlocksize) <
    ...
    for (let i = 0; i <
        Math.min(scoredFinderPatternPositions.length, MAX_FINDERPATTERNS_TO_SEARCH); ++i) {
...
function findAlignmentPattern(matrix, alignmentPatternQuads, topRight, topLeft, bottomLeft) {
    ...
    const modulesBetweenFinderPatterns = (distance(topLeft, bottomLeft) + distance(topLeft, topRight)) /
        2 /
        moduleSize;
