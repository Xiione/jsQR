...
describe("decode", () => {
  ...
  it("decodes an alphanumeric code", async () => {
    const data = await loadBinarized(
      "./src/decoder/test-data/alphanumeric.png",
    );
    expect(decode(data)).toEqual({
      text: "ABCD1234",
  ...
  it("decodes a mixed code", async () => {
    ...
    expect(decode(data)).toEqual({
      ...
      bytes: [
        49, 50, 51, 52, 53, 54, 55, 56, 57, 65, 66, 67, 68, 49, 50, 51, 52, 84,
        101, 115, 116, 131, 101, 131, 88, 131, 103,
      ],
      chunks: [
  ...
  it("returns null if there aren't enough codewords in the matrix", async () => {
    ...
    const d = Uint8ClampedArray.from(
      await fs.readFile("./src/decoder/test-data/truncated-null.bin"),
    );
  ...
  it("returns null if there aren't enough codewords in the matrix", async () => {
    ...
    const d = Uint8ClampedArray.from(
      await fs.readFile("./src/decoder/test-data/truncated-corrupted.bin"),
    );
    const matrix = new BitMatrix(d, Math.sqrt(d.length));
    expect(decode(matrix)).toBeNull();
  });
  ...
  it("Supports ECI chunks", async () => {
    ...
    expect(decode(data)).toEqual({
      ...
      bytes: [
        55, 57, 52, 56, 44, 51, 50, 56, 44, 49, 48, 49, 57, 44, 49, 52, 57, 44,
        49, 50, 44, 49, 50, 44, 49, 53, 44, 52, 44, 49, 52, 44, 49, 49, 44, 51,
        50, 44, 52,
      ],
      chunks: [
        {
        ...
        },
        {
          type: "byte",
          bytes: [
            55, 57, 52, 56, 44, 51, 50, 56, 44, 49, 48, 49, 57, 44, 49, 52, 57,
            44, 49, 50, 44, 49, 50, 44, 49, 53, 44, 52, 44, 49, 52, 44, 49, 49,
            44, 51, 50, 44, 52,
          ],
          text: "7948,328,1019,149,12,12,15,4,14,11,32,4",
        },
  ...
  it("Extracts a QR code that is missing the termination byte", async () => {
    const data = await loadBinarized(
      "./src/decoder/test-data/no-termination-byte.png",
    );
    expect(decode(data)).toEqual({
      text: "1788c74b1c9262866c2071b65df7bfcb7911c2b064c931b580515c2d9d2cd7f8",
      bytes: [
        49, 55, 56, 56, 99, 55, 52, 98, 49, 99, 57, 50, 54, 50, 56, 54, 54, 99,
        50, 48, 55, 49, 98, 54, 53, 100, 102, 55, 98, 102, 99, 98, 55, 57, 49,
        49, 99, 50, 98, 48, 54, 52, 99, 57, 51, 49, 98, 53, 56, 48, 53, 49, 53,
      ...
      chunks: [
        ...
        { type: "numeric", text: "9262866" },
        {
          type: "byte",
          bytes: [
            99, 50, 48, 55, 49, 98, 54, 53, 100, 102, 55, 98, 102, 99, 98, 55,
            57, 49, 49, 99, 50, 98, 48, 54, 52, 99, 57, 51, 49, 98,
          ],
  ...
  it("Supports StructuredAppend chunks", async () => {
    const data = await loadBinarized(
      "./src/decoder/test-data/structuredAppend.png",
    );
    expect(decode(data)).toEqual({
      text: "Hello ",
